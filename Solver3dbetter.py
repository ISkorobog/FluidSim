...
class Solver:
    ...
    def solve(self):
        ...
        for i in xrange(kw):
            ...
            c.append(self.workers[i].getnode(nodes[i].u,nodes[i].v,nodes[i].w,nodes[i].first,rho,nu,dt,dx,dy,dz,i,step,left,right,kw))
        ...
        while stepcount<nit:
            ...
            c=[]
            for i in xrange(kw):
                c.append(self.workers[i].getuvw())
            Solver.interactions+=1
            self.checkInteractions()
            c
            stepcount+=1 
    ...
    @staticmethod
    @expose
    def getnode(u,v,w,first,rho,nu,dt,dx,dy,dz,num,step,left,right,kw):
        Solver.u=u
        Solver.v=v
        Solver.w=w
        Solver.first=first
    ...
    def getp(pvalue):
        if Solver.num==0:
            Solver.p[0]=copy.deepcopy(Solver.p[1])
        else:
            Solver.p[0]=Solver.left.getrightp()
        if Solver.num==Solver.kw-1:
            Solver.p[-1]=copy.deepcopy(Solver.p[-2])
        else:
    ...
    def getuvw():
        if Solver.num==0:
            Solver.u[0]=[[0 for i in range(Solver.nz)] for j in range(Solver.nx)]
            Solver.v[0]=[[0 for i in range(Solver.nz)] for j in range(Solver.nx)]
            Solver.w[0]=[[0 for i in range(Solver.nz)] for j in range(Solver.nx)]
        else:
            (Solver.u[0],Solver.v[0],Solver.w[0])=Solver.left.getrightuvw()
        if Solver.num==Solver.kw-1:
            Solver.u[-1]=[[0 for i in range(Solver.nz)] for j in range(Solver.nx)]
            Solver.v[-1]=[[0 for i in range(Solver.nz)] for j in range(Solver.nx)]
            Solver.w[-1]=[[0 for i in range(Solver.nz)] for j in range(Solver.nx)]
        else:
            (Solver.u[-1],Solver.v[-1],Solver.w[-1])=Solver.right.getleftuvw()
        Solver.interactions+=1
    @staticmethod
    @expose
    def getleftuvw():
        return (Solver.u[0],Solver.v[0],Solver.w[0])
    @staticmethod
    @expose
    def getrightuvw():
        return (Solver.u[-1],Solver.v[-1],Solver.w[-1])
    @staticmethod
    @expose
    def getleftu(uw):
        Solver.u[0]=list(uw)
